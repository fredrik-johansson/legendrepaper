...
functions and permits using Algorithm~\ref{alg:hyprs} below to evaluate the series.
Let $\omega = 1 - (x/y) i$, with $x = \cos(\theta)$ and $y =
\sin(\theta)$ as usual.
Then
\begin{equation}
...
where $c'_{n,k}/c'_{n,k-1} = (n-k)(n+k+1)/(k(k+1))$ and $c'_{n,0} = n(n+1) / 2$.
