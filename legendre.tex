...
\headers{Rigorous evaluation of Legendre polynomials}{Fredrik Johansson, Marc Mezzarobba}

\title{Fast and rigorous arbitrary-precision evaluation of Legendre
polynomials and Gauss-Legendre quadrature nodes and weights}
...
\Cref{sec:bench} presents benchmark results
...
the three-term recurrence \eqref{eq:recurrence},
...
where \eqref{eq:pzeroseries0} and \eqref{eq:pzeroseries1}
...
the bottom sum in
\eqref{eq:pzeroseries0} or \eqref{eq:pzeroseries1} (with prefactors removed)
...
The alternating series \eqref{eq:pzeroseries0} and \eqref{eq:pzeroseries1}
...
\end{equation}
As in the previous section, we can truncate \eqref{eq:poneseries} and
bound the error by comparison with a geometric series.

\begin{proposition}
\label{prop:poneseries}
The error when truncating \eqref{eq:poneseries} after the $k = K - 1$ term satisfies
\begin{equation}
\label{eq:truncerr2}
...
For $u \ge 0$, \eqref{eq:poneseries} does not suffer from cancellation.
...
\end{equation}
of Proposition~\ref{prop:poneseries} holds with $u$ and $\alpha$ as above.
...
\eqref{eq:asymprec}, \eqref{eq:pzerorec},
\eqref{eq:poneprec}, \eqref{eq:ponerecprime}.
...
Figure~\ref{fig:timeplot} illustrates how the time to evaluate
...
When $p \propto n$ (the three remaining plots in Figure~\ref{fig:benchplot}), the main algorithm
