...
Bernstein (\cite{borwein2012polynomials}, E.5):

$$|p^{(m)}(x)| \le \left(\frac{2n}{\sqrt{1-x^2}}\right)^m$$

Markov:
$$|p^{(m)}(x)| \le (n(n-1)\cdots(n-m+1))^2$$


$$|P_n'(x)| \le \min\left(\frac{n(n+1)}{2}, \frac{n}{\sqrt{1-x^2}}\right)$$

$$|P_n''(x)| \le \min\left(\frac{(n+2)(n+1)n(n-1)}{8}, \frac{2|x||P'(x)| + n(n+1)}{1-x^2}\right)$$

We need to quickly estimate the magnitude of binomial coefficients
...
consider Gaussian quadrature nodes for different
weight functions.
...
but only requires an FFT computation with $O(n \log n)$ arithmetic operations.
