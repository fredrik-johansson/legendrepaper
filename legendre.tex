...
Our algorithm outperforms that of Theorem~\ref{thm:complexity} for
...
directly to improve numerical stability for $x$ close to 1.
...
First, Algorithm~\ref{alg:gmprec} does not keep track of round-off
errors internally: instead, we prove an a priori bound on the
accumulated error (Corollary~\ref{cor:bound-gmprec}) and add it to the
...
necessary) and use a first-order bound
...
\ref{step:coeff1}~and~\ref{step:coeff2} according to formulas
...
git repository.
Except where otherwise noted, we ran the programs on a
...
We use the formulas in \cite[Theorem~1(c)]{petras1999computation} to
