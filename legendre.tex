...
void legendre(mpz_t p, mpz_t q, int n, const mpz_t x, int t) {
    ...
    mpz_set_ui(p, 1); mpz_mul_2exp(p, p, t);
    mpz_set(q, x);
    for (k = 1; k < n; k++) {
        mpz_mul(tmp, q, x); mpz_tdiv_q_2exp(tmp, tmp, t);
        mpz_mul_si(p, p, -k*k); mpz_addmul_ui(p, tmp, 2*k+1);
        mpz_swap(p, q);
        if (mpn_mul_1(&denlo, &den, 1, k+1)) {
            mpz_tdiv_q_ui(p, p, den); mpz_tdiv_q_ui(q, q, den);
            den = k+1;
        } else den = denlo;
    }
